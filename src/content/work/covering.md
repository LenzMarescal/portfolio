---
title: Coverings automobile - Espace Centre - 11/2025
publishDate: 2025-11-20 00:00:00
img: /assets/bus.jpg
img_alt: Photographie réelle d’un véhicule sur lequel sont posés des stickers
description: |
 Dans le cadre d’une mission de communication visuelle pour l’Espace Centre, j’ai réalisé la maquette complète du covering d’un véhicule utilitaire, destinée à l’impression et à la pose de stickers grand format.
tags:
  - Maquette
  - Photoshop
  - ADLC
  - Novembre 2025
filtre: 
  design
---
L’objectif du projet était de concevoir un habillage visuel attractif, moderne et facilement identifiable pour le Ludobus, un service itinérant dédié au jeu et à l’animation. Le design devait refléter l’univers ludique tout en restant cohérent avec l’identité graphique existante.

Mon travail, itéré plusieurs fois après apréciation du client a compris :

- La création de la maquette sur Photoshop, en prenant en compte les contraintes techniques liées au covering (courbes du véhicule, zones sensibles, marges de sécurité, proportions) ;
- L’intégration et la composition graphique de certains éléments visuels, tels que des courbes en pointillés évoquant le dynamisme, ainsi que des bulles d’informations utiles (adresse et numéro de téléphone) ;
- L’adaptation du design sur plusieurs faces du véhicule (côtés, arrière, capot), avec une cohérence visuelle et une bonne lisibilité ;
- La vectorisation de certains logos partenaires ;
- La préparation des fichiers prêts pour l’impression selon les normes des ateliers de fabrication (résolution, calques dédiés), ainsi que la préparation d’un PDF de présentation et de planches d’assets.

Le résultat est un covering dynamique, coloré et professionnel, pensé à la fois pour attirer l’attention du public et pour représenter clairement la mission du Ludobus sur le terrain.

<img src="/assets/bus.jpg">